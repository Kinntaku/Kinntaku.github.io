...
# 在文件的最顶端添加 Server = https://mirrors.ustc.edu.cn/archlinux/$repo/os/$arch
```
...
2. 启用仓库
   ...
   **注意：请参考 [CachyOS Wiki](https://wiki.cachyos.org/features/optimized_repos/) 选择适合当前设备架构的仓库！错误的软件仓库可能导致系统不稳定或无法正常工作！**
   ...
   修改 `/etc/pacman.conf`, 添加以下内容到仓库中 `[core]` 项之前
...
3. 修改镜像站
   ...
   # 在最顶端添加 Server = https://mirrors.ustc.edu.cn/cachyos/repo/$arch/$repo

   nano /etc/pacman.d/cachyos-v3-mirrorlist
   # 在最顶端添加 Server = https://mirrors.ustc.edu.cn/cachyos/repo/$arch_v3/$repo

   nano /etc/pacman.d/cachyos-v4-mirrorlist
   # 在最顶端添加 Server = https://mirrors.ustc.edu.cn/cachyos/repo/$arch_v4/$repo
   ```
...
# 修改配置 GRUB_CMDLINE_LINUX_DEFAULT ="loglevel=5 nowatchdog modprobe.blacklist=sp5100_tco"
# 启用 GRUB_DISABLE_OS_PROBER=false
...
|power-profiles-daemon|电源|
|vulkan-icd-loader|Vlkan 管理|
|snapper btrfs-assistant|Btrfs 快照工具||
|grub-btrfs inotify-tools|(可选) Grub 的 Btrfs 工具, 防滚挂|
|asusctl rog-control-center|(可选) 华硕相关|
|plasma-meta konsole dolphin egl-wayland ark kleopatra partitionmanager|kde 以及相关工具||
|sof-firmware alsa-firmware alsa-ucm-conf|声音相关|
|fcitx5-im fcitx5-chinese-addons fcitx5-rime fcitx5-configtool|输入法||
|wqy-zenhei noto-fonts noto-fonts-cjk noto-fonts-emoji noto-fonts-extra|字体|
|ntfs-3g|NTFS驱动|
|paru|AUR相关|
|starship|(可选) 终端美化|
|firewalld|(可选) 防火墙|
...
# 添加账户
useradd -m -G wheel,uccp -s /bin/zsh kinntaku # 笔者需要嵌入式开发, 添加了 uccp 组, 普通用户可以不添加
passwd kinntaku
...
pacman -Rns discover sddm-kcm sddm # (可选) 卸载 kde 自带的 sddm
...
```
# (可选)安装提前准备的 flclash
sudo paru -U flclash-bin-0.8.90-1-x86_64.pkg.tar.zst
...
|ps-office-cn wps-office-mui-zh-cn|(可选) wps相关|
|wemeet-bin linuxqq wechat-bin|(可选) 通讯工具|


```
"${SHELL}" <(curl -L micro.mamba.pm/install.sh) # (可选) 安装 micromamba

# (可选) 安装 nvidia-laptop-power-cfg 
...
```
cat /proc/driver/nvidia/gpus/bus_address/power # 查看显卡是否动态启动, 其中 bus_address 需要换成具体数值
```
...
## console(miniconda starship distrobox vim)
...
## stop usb device wake up laptop
...
## chromium
--use-spdy=off
https://chromewebstore.google.com/detail/%E5%B0%8F%E8%88%92%E5%90%8C%E5%AD%A6-%E5%9F%BA%E4%BA%8E%E4%B9%A6%E7%AD%BE%E7%9A%84%E6%96%B0%E6%A0%87%E7%AD%BE%E9%A1%B5/eldcinofoklpfhaanlhmkmadehfgcnon
https://chromewebstore.google.com/detail/bitwarden-%E5%AF%86%E7%A0%81%E7%AE%A1%E7%90%86%E5%99%A8/nngceckbapebfimnlniiiahkandclblb
https://chromewebstore.google.com/detail/allow-copy-+/ajhbdcgfhlhhmocddefknjjkejcfpbnj
...
sudo apt install build-essential curl wget binutils-arm-none-eabi gcc-arm-none-eabi libstdc++-arm-none-eabi-newlib gdb-multiarch
...
sudo vim /boot/grub/themes/minegrub-world-selection/theme.txt
top + 45
