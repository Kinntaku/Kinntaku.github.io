...
6. 部分软件安装包
   ...
   - (推荐) [XnViewMP](https://www.xnview.com/en/xnview-mp/): 一款免费的专业图片浏览和管理工具

   - [Visual Studio Code](https://code.visualstudio.com/Download)

   - (可选) [Mujoco](https://github.com/google-deepmind/mujoco/releases): 机器人仿真平台

   - (可选) [STM32CubeMX](https://www.st.com/en/development-tools/stm32cubemx.html): 意法半导体单片机配置工具

5. 需要的 AppImage

   - (推荐) [Motrix](https://motrix.app/download): 一款下载器
   
   - (可选) [QQ 音乐](https://y.qq.com/download/download.html)

   - (可选) [PureRef](https://www.pureref.com/download.php): 一款绘画参考图查看工具

      Linux 下载中选择 `Portable` 然后 `Select amount` 中选择第三个 `Custom Amount` 输入 `0` 即可下载免费版

   - (可选) [QtScrcpy](https://github.com/barry-ran/QtScrcpy/releases): 一款安卓投屏工具

   - [微信](https://linux.weixin.qq.com/)

   PS: 腾讯 QQ 也提供了 AppImage 版本, 但是由于打包问题, 无法使用 AppImageLauncher 安装, 建议直接使用 AUR 包

6. 字体文件

   - (可选) [Nerd Fonts](https://www.nerdfonts.com/font-downloads): 用于终端美化显示系统图标等

      这里选择 `0xProto Nerd Font`

   - (可选) [Jetbrains Mono](https://www.jetbrains.com/lp/mono/): 好看的编程字体
   
   - (可选) Windows 字体: 防止wps和qq音乐缺少字体

      从 Windows 系统中拷贝 `C:\Windows\Fonts` 文件夹下的字体文件到一个单独的文件夹中备用
...
1. 导入 CachyOS 的 GPG 密钥并安装相关包
   ...
   注: 密钥可能会更新, 请进入 [CachyOS 官方文档](https://wiki.cachyos.org/features/optimized_repos/) - `Adding Our Repositories to an Existing Arch Linux Install` - `Manual` 选项 - `Install CachyOS keyring:` 查看最新指令
   ...
   注: 包可能会更新, 请进入 [CachyOS 官方文档](https://wiki.cachyos.org/features/optimized_repos/) - `Adding Our Repositories to an Existing Arch Linux Install` - `Manual` 选项 - `Install the necessary packages:` 查看最新指令

   注: 如果官方文档也没来得及更新, 可以去 cahcyos 仓库, 比如 [中科大镜像站](https://mirrors.ustc.edu.cn/cachyos) 搜索出错的包, 查看具体版本, 修改安装指令
...
# 安装基础组件
pacstrap -K /mnt base base-devel linux-cachyos linux-cachyos-headers linux-firmware amd-ucode btrfs-progs networkmanager gvim sudo zsh zsh-completions =grub efibootmgr os-prober openssh
```
...
|grub efibootmgr os-prober|引导工具,本教程使用 grub, 也可以使用 rEFInd, systemd-boot 或 limine||
...
***想使用 Hyprland 的用户到这里就可以跳转到 [Hyprland 使用教程](https://kinntaku.github.io/blog/hyprland/) 继续安装了***
...
Plasma 设置 - 文字和字体 - 字体管理 - 安装字体 - 系统字体
...
```bash
sudo pacman -S podman xorg-xhost lazydocker distrobox nvidia-container-toolkit picocom cmake jdk21-openjdk obs-studio chromium virt-manager pandoc vlc ffmpeg vlc-plugin-ffmpeg steam blender gimp krita kdenlive localsend
sudo paru -S appimagelauncher wps-office-cn wps-office-mui-zh-cn wemeet-bin linuxqq
...
|localsend|(可选) 局域网文件传输工具|
|appimagelauncher|(可选) AppImage安装器|
|wps-office-cn wps-office-mui-zh-cn|(可选) wps相关|
|wemeet-bin linuxqq|(可选) 通讯工具|

### 使用 AppImageLauncher 安装 AppImage
...
Plasma设置 - 会话 - 桌面会话 - 登录时自动启动应用程序: 启动为空会话
...
```json
"editor.fontFamily": "JetBrains Mono, 'Droid Sans Mono', 'monospace', monospace, 'MesloLGS NF'",
```

### (可选) podman 和 distrobox 设置
...
- 创建带cuda的Ubuntu容器
   ...
   export MAMBA_EXE='/home/kinntaku/.local/bin/micromamba';
...
## 安装字体导致字体 fallback 链出问题

创建

```bash
vim /etc/fonts/conf.d/99-my-global.conf
```
...
<fontconfig>
  ...
  <!-- 强制重写 generic family，让它进入 alias 阶段 -->
